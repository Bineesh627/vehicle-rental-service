...
export const chatApi = {
     ...
     */
    async getOrCreateConversation(token: string, shopId: string): Promise<ChatConversation> {
        const resp = await fetch(`${API_BASE_URL}/chat/conversations/`, {
            method: 'POST',
            headers: authHeaders(token),
     ...
     */
    async getMessages(token: string, conversationId: string, myUserId: string): Promise<ChatMessage[]> {
...
export const profileManagementApi = {
    ...
    async getUserProfileExtended(): Promise<UserProfile & { settings: UserSettings }> {
        const token = await getAuthToken();
        if (!token) throw new Error('No authentication token found');
    ...
    async createPaymentMethod(paymentData: PaymentMethodCreate): Promise<PaymentMethod> {
        const token = await getAuthToken();
        if (!token) throw new Error('No authentication token found');
    ...
    async updatePaymentMethod(id: string, paymentData: Partial<PaymentMethodCreate>): Promise<PaymentMethod> {
        const token = await getAuthToken();
        if (!token) throw new Error('No authentication token found');
    ...
    async createSavedLocation(locationData: Omit<SavedLocation, 'id' | 'created_at'>): Promise<SavedLocation> {
        const token = await getAuthToken();
        if (!token) throw new Error('No authentication token found');
    ...
    async updateSavedLocation(id: string, locationData: Partial<SavedLocation>): Promise<SavedLocation> {
        const token = await getAuthToken();
        if (!token) throw new Error('No authentication token found');
    ...
    async updateKYCDocument(kycData: Partial<KYCDocumentCreate>): Promise<KYCDocument> {
        const token = await getAuthToken();
        if (!token) throw new Error('No authentication token found');
